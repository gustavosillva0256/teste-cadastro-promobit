	...
	<table id="example" class="table table-hover table-sm my-4" style="width:98%;">
		...
		<tbody>
			...
			for($i=0; $i < @count($res); $i++){
				...
				$query2 = $pdo->query("SELECT * FROM tags where id = '$id_tag'");
									...
									<div class="col-4">
										<div class="mb-3">
											...
											<select class="form-select" aria-label="Default select example" name="tag">
												<?php 
												$query = $pdo->query("SELECT * FROM tags order by nome asc");
												$res = $query->fetchAll(PDO::FETCH_ASSOC);
												for($i=0; $i < @count($res); $i++){
													foreach ($res[$i] as $key => $value){	}
														$id_item = $res[$i]['id'];
													...
													?>
													<option <?php if(@$id_item == @$tag){ ?> selected <?php } ?> value="<?php echo $id_item ?>"><?php echo $nome_item ?></option>
